<cfcomponent extends="mxunit.framework.TestCase">
	...
	* Does it call delete on those old files?
	...
	<cffunction name="runCleanupMaintenanceWithNoValidDirectoriesReturnsNoResults">
		...
		<cfset assertEquals(0,ArrayLen(flattenResultsStruct(results).deletedFiles),"none of the spoof directories exist and therefore no files should be marked for deletion")>
	</cffunction>  
	...
	<cffunction name="runCleanupMaintenanceShouldHitExpectedFiles">
		...
		<cfset assertEquals(3,StructCount(results),"spoof query contained 3 configurations, but the result struct had #StructCount(results)#")>
		<cfset flattenedResults = flattenResultsStruct(results)>
		<cfset fileList = ArrayToList(flattenedResults.deletedFiles)>
	...
	<cffunction name="runCleanupMaintenanceShouldNotFailOnFileDeleteErrors">
		...
		<cfset results = fsm.runCleanupMaintenance()>
		<cfset flattenedResults = flattenResultsStruct(results)>
		<cfset assertEquals(0, ArrayLen(flattenedResults.deletedfiles) )>
		<!--- 3 because that's how many files we know should've been deleted --->
		<cfset assertEquals(3,ArrayLen(flattenedResults.errors))>
		<cfset assertTrue( StructKeyExists( flattenedResults.errors[1],"TagContext" ) )> 
	</cffunction>
	...
	<cffunction name="flattenResultsStruct" access="private">
		<cfargument name="resultsStruct">
		<cfset var configid = "">
		<cfset var flattened = {deletedFiles=ArrayNew(1),errors=ArrayNew(1)}>
		<cfloop collection="#resultsStruct#" item="configid">
			<cfset flattened.deletedFiles.addAll(resultsStruct[configid].deletedFiles)>
			<cfset flattened.errors.addAll(resultsStruct[configid].errors)>
		</cfloop>
		<cfreturn flattened>
	</cffunction>
