<cfcomponent extends="mxunit.framework.TestCase">
	...
	<cffunction name="myTimelineShouldReturnMyTweetsArray">
		...
		<cfset assertTrue( arrayLen(results) lte 20 , '')>
	...
	<cffunction name="friendsTimelineShouldReturnFriendsTweetsArray">
		...
		<cfset assertTrue( isNumeric(results[arraylen(results)].id) , "The id in the results is NOT numeric.") />
		<cfset assertTrue( structKeyExists(results[arraylen(results)],"user") , "The USER key wwas not found in the struct.") />
	</cffunction>
